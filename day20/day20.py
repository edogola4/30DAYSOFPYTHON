...
for cat in cats:
    # Handle weight
    if 'weight' in cat and 'weight_metric' in cat['weight']:
        weight_range = cat['weight']['metric'].split('-')
        try:
            weights.append(float(weight_range[0].strip()))
        except ValueError:
            continue

    # Handle lifespan
    if 'life_span' in cat:
        lifespan = cat['life_span'].replace('–', '-').split('-')
        try:
            if len(lifespan) == 2:
                lifespans.append((int(lifespan[0].strip()), int(lifespan[1].strip())))
            elif len(lifespan) == 1:
                lifespan_value = int(lifespan[0].strip())
                lifespans.append((lifespan_value, lifespan_value))
        except ValueError:
            continue

# Converting to numpy arrays for calculations
weights = np.array(weights)
lifespans = np.array([np.mean(span) for span in lifespans])
...
weight_stats = calculate_stats(weights)
lifespan_stats = calculate_stats(lifespans)

print("\nCat Weight Statistics (kg):")
if weight_stats['min'] is None:
...
# Extracting country names and languages
country_names = [country['name'] for country in countries]
languages = [lang for country in countries for lang in country.get('languages', [])]

# Extracting the most spoken languages
language_names = [language['name'] for language in languages]
language_counts = Counter(language_names)

# Finding the 10 largest countries
sorted_countries = sorted(countries, key=lambda x: x.get('area', 0), reverse=True)
largest_countries = [(country['name'], country['area']) for country in sorted_countries[:10]]
...
print("\nTotal Number of Languages:", len(language_counts))
...
# Fetching the UCL dataset page
url = 'https://archive.ics.uci.edu/ml/datasets.php'
response = requests.get(url)
soup = BeautifulSoup(response.text, 'html.parser')

# Finding dataset titles
datasets = soup.find_all('td', class_='title')
dataset_titles = [dataset.text.strip() for dataset in datasets]
